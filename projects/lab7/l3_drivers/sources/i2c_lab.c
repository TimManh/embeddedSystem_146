...
void i2c_lab_init(uint8_t slave_address_not_reg_address) {
  LPC_I2C2->ADR0 |=
...
void i2c_lab_init_loopback(uint8_t slave_address_not_reg_address) {
  LPC_I2C1->ADR0 |=
      (slave_address_not_reg_address << 0); // set the address of slave to any number passed by user in main
...
bool i2c_write_lab(uint8_t mem_location, uint8_t data_write) {
...
bool i2c_read_lab(uint8_t mem_location, uint8_t *data_pointer_read) {
  bool read_status;
  if (mem_location > 256 || mem_location < 0) {
    fprintf(stderr, "Out of bound memory");
    read_status = false;
